...
const Calendar = () => {
  ...
  const fetchHolidays = async () => {
    try {
      const response = await axios.get('http://127.0.0.1:8080/api/holidays');
  ...
  const handleSubmitHoliday = async () => {
    if (newHoliday.name && selectedDate) {
      try {
        const response = await axios.post('http://127.0.0.1:8080/api/holidays', {
  ...
  const handleRemoveHoliday = async (holidayId) => {
    try {
      ...
      await axios.delete(`http://127.0.0.1:8080/api/holidays/${holidayId}`);
